...
specan3 <-
  ...
  drop <- c('duration','sound.files','selec','peakf')
  
  acoustics = acoustics[,!(names(acoustics) %in% drop)]
  View(acoustics)
  write.table(acoustics,"voiceDetails.csv",sep=",",row.names = FALSE)
  return (acoustics)
}
